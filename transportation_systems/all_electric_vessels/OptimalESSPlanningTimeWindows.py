...
class OptimalPlanningESS():
    ...
    def problem_formulaiton(self, networks=transportation_network()):
        ...
        #
        ns = 100
        delta = 0.1 # The derivation range
        random.seed(0)
        ...
        for i in range(T):
            ...
            for j in range(NYs):
                lb[i * NX + I_C_H0 + j] = 0
                ub[i * NX + I_C_H0 + j] = 1  # Do not allow half cruise
                c[i * NX + I_C_H0 + j] = 0
            ...
            for j in range(ng):
                ...
                ub[i * NX + I_G0 + j] = 1
                c[i * NX + I_G0 + j] =  a0[j]
                c_emission[i * NX + I_G0 + j] = 0.0845 * PMAX[j] * 0.846
                c_fuel[i * NX + I_G0 + j] = a0[j]
            ...
            for j in range(ng):
                ...
                c[i * NX + P_G0 + j] =  a1[j]
                c_emission[i * NX + P_G0 + j] = 0.245 * 0.846
                c_fuel[i * NX + P_G0 + j] = a1[j]
            # PESS_DC
            lb[i * NX + PESS_DC] = 0
        ...
        c[ESSCAP] = CostEssE
        c_ess[ESSCAP] = CostEssE
        vtypes[ESSCAP] = 'd'

        lb[PESSCAP] = 0
        ub[PESSCAP] = PcapacityEss
        c[PESSCAP] = CostEssP
        c_ess[PESSCAP] = CostEssP
        vtypes[PESSCAP] = 'd'
        ...
        # equation 11, the status constraint
        Aeq_temp = zeros((T, nx))
        beq_temp = zeros(T)
        for i in range(T):
            for j in range(NPORTs):
        ...
        # for i in range(T):
        #     for j in range(NPORTs):
        #         A_temp[i, i * NX + I_S + j] = 1
        #     for j in range(NYs):
        #         A_temp[i, i * NX + I_A0 + j] = 1
        #         A_temp[i, i * NX + I_D0 + j] = 1
        #         A_temp[i, i * NX + I_C0 + j] = 1
        #     A_temp[i, i * NX + It] = -1
        # A = vstack([A, A_temp])
        # b = concatenate([b, b_temp])
        ...
        df = pd.DataFrame(array([solution["fuel"]]))
        df.to_excel(writer, sheet_name='fuel')
        df = pd.DataFrame(array([solution["ess"]]))
        df.to_excel(writer, sheet_name='ess')
