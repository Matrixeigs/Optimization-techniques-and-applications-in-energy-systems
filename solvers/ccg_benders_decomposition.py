    ...
    def main(self, c=None, A=None, b=None, Aeq=None, beq=None, lb=None, ub=None, vtype=None, ps=None, qs=None, Ws=None,
        ...
        for i in range(self.N):
            model_second_stage[i] = {"c": qs[i],
                                     ...
                                     "ub": ones((self.nx_second_stage, 1)) * inf}
        ...
        while iter < iter_max:
            ...
            neq = model_second_stage[index[0]]["Aeq"].shape[0]
            Aeq_temp = zeros((neq, nx_temp))
            Aeq_temp[:, 0:self.nx_first_stage] = model_second_stage[index[0]]["Ts"]
            Aeq_temp[:, nx:] = model_second_stage[index[0]]["Aeq"]

            if model_master["Aeq"] is not None:
                model_master["Aeq"] = hstack(
                    [model_master["Aeq"], zeros((model_master["Aeq"].shape[0], self.nx_second_stage))])
                model_master["Aeq"] = vstack([model_master["Aeq"], Aeq_temp])
                model_master["beq"] = vstack([model_master["beq"], model_second_stage[index[0]]["hs"]])
            else:
                model_master["Aeq"] = Aeq_temp
                model_master["beq"] = model_second_stage[index[0]]["hs"]
